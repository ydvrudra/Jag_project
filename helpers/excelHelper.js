...
// function getTaxableAmount(f) {
//   return parseAmount(
//     f["TAXABLE AMOUNT"]?.valueString ??
//     f["TAXABLE AMOUNT"]?.content ??
//     f["TAXABLE_AMOUNT"]?.valueString ??
//     f["TAXABLE_AMOUNT"]?.content ??
//     f["AMOUNT"]?.valueString ??
//     f["AMOUNT"]?.content ??
//     f["VALUE"]?.valueString ??
//     f["VALUE"]?.content ??
//     0
...
//   return supplier.includes("hapag") || supplier.includes("lloyd");
// }

// // ---------- Hapag Tax Mapping ----------
// function applyHapagLloydTaxMapping(f) {
//   const mapped = { ...f };

//   const igstCode = f["TAX"]?.content || "";   // 🟢 use TAX field for IGST/CGST/SGST codes
//   if (igstCode === "G0") {
//     mapped["IGST %"] = "5";
//   } else if (igstCode === "G2") {
//     mapped["IGST %"] = "18";
...
//       let taxable = 0, igstPct = 0, cgstPct = 0, sgstPct = 0;

//       if (isHapag) {
//          // ✅ Hapag Lloyd invoices: taxable amount field use karo (RATE fallback sirf USD case ke liye)
//       taxable = getTaxableAmount(f);
//      if (!taxable) {
//       taxable = parseAmount(f["RATE"]?.valueString ?? f["RATE"]?.content);
//      }

//         const taxCode = f["TAX"]?.content || "";
//         if (taxCode === "G0") igstPct = 5;
//         if (taxCode === "G2") igstPct = 18;
//         if (taxCode === "C0") { cgstPct = 2.5; sgstPct = 2.5; }
//         if (taxCode === "C2") { cgstPct = 9; sgstPct = 9; }
//       } else {
//         // Other vendors ka data direct field se lena
//           taxable = getTaxableAmount(f);
//         taxable = parseAmount(f["TAXABLE AMOUNT"]?.valueString ?? f["TAXABLE AMOUNT"]?.content);
//         igstPct = parseAmount(f["IGST %"]?.content || f["IGST %"]);
//         cgstPct = parseAmount(f["CGST %"]?.content || f["CGST %"]);
//         sgstPct = parseAmount(f["SGST %"]?.content || f["SGST %"]);
//       }

//       // Calculate tax amounts
//       const igstAmt = (igstPct / 100) * taxable;
//       const cgstAmt = (cgstPct / 100) * taxable;
//       const sgstAmt = (sgstPct / 100) * taxable;
